...
if __name__ == "__main__":
    ...
    for i in range(1, num_test_files + 1):
        ...
        try:
            ...
            correct_answers = [
                line.strip().split() for line in correct_file.readlines()
            ]
            ...
            while i < num_tokens:
                if correct_answers[i][0] == "INVALID":
                    try:
                        ...
                        print(
                            f"Token {i}: Incorrect.  Should have raised InvalidToken exception"
                        )
                    ...
                    try:
                        ...
                        res = (
                            "Correct"
                            if token[0] == correct_answers[i][0]
                            and token[1] == correct_answers[i][1]
                            else "Incorrect"
                        )
                        ...
                        if res == "Correct":
                            ...
                            print(
                                f"correct token type = {correct_answers[i][0]}: your token type = {token[0]}"
                            )
                            print(
                                f"correct token value = {correct_answers[i][1]}: your token value = {token[1]}"
                            )
                    except pa4.InvalidToken:
                        print(
                            f"Token {i}: Incorrect.  Should not have raised InvalidToken exception"
                        )
                i += 1
            ...
            if correct_answers[i - 1][0] != "INVALID":
                try:
                    token = lex.next_token()
                    print(
                        "Your code did not raise an EOFError exception when it should have"
                    )
